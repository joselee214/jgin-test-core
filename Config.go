...
func (cfg *Config) Parse(fpath string) {
	...
	for k, v := range cfg.All {
		if strings.Index(k, "jgin.app.") == 0 {
			tmp := strings.TrimPrefix(k, "jgin.app.")
			cfg.App[tmp] = v
		} else if strings.Index(k, "jgin.session.") == 0 {
			tmp := strings.TrimPrefix(k, "jgin.session.")
			cfg.Session[tmp] = v
		} else if strings.Index(k, "jgin.logger.") == 0 {
			tmp := strings.TrimPrefix(k, "jgin.logger.")
			cfg.Logger[tmp] = v
		} else if strings.Index(k, "jgin.view.") == 0 {
			tmp := strings.TrimPrefix(k, "jgin.view.")
			cfg.View[tmp] = v
		} else if strings.Index(k, "jgin.static.") == 0 {
			tmp := strings.TrimPrefix(k, "jgin.static.")
			cfg.Static[tmp] = v
		} else if strings.Index(k, "jgin.staticfile.") == 0 {
			tmp := strings.TrimPrefix(k, "jgin.staticfile.")
			cfg.StaticFile[tmp] = v
		} else if strings.Index(k, "jgin.datasource.") == 0 {
